...
The trigger only fires if Custom Settings value: causeview__BatchSettings__c.causeview__Data_Migration_Mode__c = FALSE
Based on the Gift Recordtype, Gift Type and Payment Type values, the trigger takes the correct GL Debit 
and GL Credit values from the Fund record (causeview__Gift_Detail__c.causeview__Fund__c) 
and populate them into causeview__GL_Auto_Credit_Account__c and causeview__GL_Auto_Debit_Account__c fields.
*/
trigger GLfieldUpdateOnAllocationTrigger on Gift_Detail__c (before insert, before update) {
    ...
    if(!BatchSettings__c.getInstance('Default').Data_Migration_Mode__c){
        ...
        Map<Id, Gift__c> giftRecordMap = new Map<Id, Gift__c>([select Id, Gift_Type__c, RecordTypeId, causeview__Gift_Date__c from Gift__c where Id IN: giftIds]);
        if(paymentIds.size() > 0 && fundIds.size() > 0)
        {            
            paymentRecordMap = new Map<Id, Payment__c>([select Id, Payment_Type__c, Payment_Refunded__c, Status__c, Date__c, RecordTypeId,RecordType.Name, (select Id from Installment_Fulfillments__r) from Payment__c where Id IN: paymentIds]);
         ...
         Map<Id, Installment__c> installment_Record_Map ;
         Map<Id, Payment__c> refundedPaymentRecordMap;
         Map<Id, Fund__c> fundRecordMap;
         if(installment_Ids.size() > 0)
            installment_Record_Map = new Map<Id, Installment__c>([select Id from Installment__c where Id IN: installment_Ids]);
         if(refundPaymentIds.size() > 0)   
            refundedPaymentRecordMap = new Map<Id, Payment__c>([select Id, Payment_Type__c, Payment_Refunded__c, Status__c, Date__c, RecordTypeId, (select causeview__Fund__c, GL_Auto_Credit_Account__c, GL_Auto_Debit_Account__c from Allocations__r) from Payment__c where Id IN: refundPaymentIds]);
         if(fundIds.size() > 0)   
            fundRecordMap = new Map<Id, Fund__c>([select GL_Credit__c, GL_Debit__c, GL_In_Kind_Credit__c, GL_In_Kind_Debit__c, GL_Matching_Pledge_Cash_Credit__c, GL_Matching_Pledge_Cash_Debit__c,
        ...
        for(Gift_Detail__c g : trigger.new){
            if(giftRecordMap.get(g.Gift__c) != null && (fundRecordMap != null && fundRecordMap.get(g.Fund__c) != null) && (paymentRecordMap!= null && paymentRecordMap.get(g.Payment__c) != null) || (installment_Record_Map != null && installment_Record_Map.get(g.Installment__c) != null)){
                ...
                System.debug(g.Fund__c);
                System.debug(fundRecordMap);
                ...
                if(paymentRecordMap != null && paymentRecordMap.get(g.Payment__c) != null){
                    ...
                    {
                     system.debug('<=Inside else edit1=>');
                        if(giftRecord.RecordTypeId == rtypes_Map.get('Gift') && giftRecord.Gift_Type__c == 'One Time Gift' && paymentRecord.Status__c == 'Approved' && (paymentRecord.Payment_Type__c == 'Cash' || paymentRecord.Payment_Type__c == 'Check' || paymentRecord.Payment_Type__c == 'Credit Card' || paymentRecord.Payment_Type__c == 'Credit Card - Offline' || paymentRecord.Payment_Type__c == 'ACH/PAD')){
                            g.GL_Auto_Credit_Account__c = fundRecord.GL_Credit__c;
                ...
                if(installment_Record_Map.get(g.Installment__c) != null){
                       system.debug('<=Inside else edit2=>');
                       system.debug('installment-----'+installment_Record_Map.get(g.Installment__c));
