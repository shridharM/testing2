trigger ReceiptUpdateOnTransaction on Gift__c (after Update) {
  ...
  Map<Id,Gift__c> newGiftMap = new Map<Id,Gift__c>(); 
   ...
   if(newGiftMap.size()>0) 
   {
     ...
     for(Receipt__c updateReceipt : [SELECT Id,Introduction_Text_Block_Rich_Text__c ,Custom_Rich_Text_3__c,Custom_Rich_Text_2__c,Custom_Rich_Text_1__c,Email_Subject__c, Body_Text_Block_Rich_Text__c,Signature_Rich_Text__c ,gift__c FROM Receipt__c where Receipt__c.Gift__c in :newGiftMap.keyset() FOR UPDATE])
       {
